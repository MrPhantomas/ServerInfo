    ...
    QObject(parent), projectName(projectName)
{
    ...
    sendInfoTimer = new QTimer;
    sendInfoTimer->setInterval(1000);
    connect(sendInfoTimer, SIGNAL(timeout()), this, SLOT(sendInfo()));
    sendInfoTimer->start();
}
...
DfInfo* Server::getDfInfo()
{
    QProcess obj;
    obj.start("df");
    obj.waitForFinished();
    if(obj.exitStatus() == QProcess::NormalExit && !obj.exitCode())
    {
        QString str(obj.readAll());
        QList<QString> lst = str.replace(QRegExp("[ ]{2,}")," ").split('\n');
        quint64 sum = 0;
        for(int i = 1; i < lst.count()-1; i++)
            sum +=  lst.at(i).split(" ")[3].toUInt();
        qDebug()<<"EmptySize = "<<sum;
        DfInfo* dfInfo = new DfInfo;
        dfInfo->emptyDfSize = sum;
        return dfInfo;
    }
    return nullptr;
}
...
{
    DfInfo* dfInfo = getDfInfo();
    if(dfInfo!=nullptr)
    {
        QByteArray tmpMessage;
        udpStruct outStrct;
        QDataStream out(&tmpMessage, QIODevice::WriteOnly);
        outStrct.command = quint8(GETDFINFO);
        outStrct.shortInfo = "t";
        QDataStream outData(&outStrct.data, QIODevice::WriteOnly);
        outData<<dfInfo->emptyDfSize;
        outStrct.numPacket = quint32(1);
        outStrct.packets = quint32(1);
        outStrct.dataSize = quint64(outStrct.data.size());
        out<<outStrct;
        udpSocket->writeDatagram(tmpMessage, tmpMessage.size(), QHostAddress::Broadcast, defUdpCLientPort);
        udpSocket->waitForBytesWritten();
    }
}
